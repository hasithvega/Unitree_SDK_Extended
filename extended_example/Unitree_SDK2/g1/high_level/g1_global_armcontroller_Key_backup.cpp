...
public:
    ...
    Eigen::Vector3f ComputeForwardKinematics(bool is_left_arm, const std::array<float, 7>& joint_positions) {
        ...
        if (is_left_arm) {
            ...
            y = left_arm_lengths_[1] * sin(shoulder_roll) + 
                left_arm_lengths_[2] * sin(shoulder_roll);
            ...
            y = -(right_arm_lengths_[1] * sin(shoulder_roll) + 
                right_arm_lengths_[2] * sin(shoulder_roll));
            z = right_arm_lengths_[1] * sin(shoulder_pitch) * cos(shoulder_roll) + 
                right_arm_lengths_[2] * sin(shoulder_pitch + elbow) * cos(shoulder_roll);
        }
    ...
    void KeyboardControl() {
        ...
        while (true) {
            arm_choice = getchar();
            if (arm_choice == 'l' || arm_choice == 'r' || arm_choice == 'b') {
                break;
            }
        ...
        // Now switch to non-blocking mode for the actual keyboard control
        ...
        // Extract left and right arm joint positions
        std::array<float, 7> left_joints, right_joints;
        for (int i = 0; i < 7; ++i) {
            left_joints[i] = current_jpos[i];
            right_joints[i] = current_jpos[i+7];
        }

        // Compute current end-effector positions
        left_ee = ComputeForwardKinematics(true, left_joints);
        right_ee = ComputeForwardKinematics(false, right_joints);
        
        // Store initial positions for reset
        const Eigen::Vector3f initial_left_ee = left_ee;
        const Eigen::Vector3f initial_right_ee = right_ee;
            ...
            if (ch != EOF) {
                switch (ch) {
                    ...
                    case 'e': case 'E':
                        ...
                        break;
                    case 'r': case 'R':
                        left_ee = initial_left_ee;
                        right_ee = initial_right_ee;
                        position_changed = true;
                        break;
                    case 27: // ESC
                    case ' ': // Space
                        MoveArmsToPosition(target_down);
                        running = false;
                        break;
                }
