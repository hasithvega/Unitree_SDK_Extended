...
void point_to_gridmap::pointCloud_callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr scanMsg){
    if (!laserOdomQueue.empty())
    {
        ...
        nav_msgs::msg::Odometry  laserOdom= laserOdomQueue.back();
        //获取激光里程计信息
        p_x = laserOdom.pose.pose.position.x;
        p_y = laserOdom.pose.pose.position.y;
        p_z = laserOdom.pose.pose.position.z;

        theta = getYaw(laserOdom);
        //获取当前激光帧的时间戳
        scanMsg_time.header.stamp = scanMsg->header.stamp;

        // pub_Current_LocalCloud(scanMsg, scanMsg_time);  
        /*****************************对点云进行滤波*******************************/
        pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_src(new pcl::PointCloud<pcl::PointXYZ>());
        pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_filtered(new pcl::PointCloud<pcl::PointXYZ>());

        pcl::PointCloud<pcl::PointXYZ> PCscan_0;
        pcl::fromROSMsg(*scanMsg, PCscan_0);
        *cloud_src = PCscan_0;
        ...
        laserCloudTemp.header.stamp = scanMsg->header.stamp;
