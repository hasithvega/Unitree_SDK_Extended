...
class BmsState(metaclass=Metaclass_BmsState):
    ...
    SLOT_TYPES = (
        ...
        rosidl_parser.definition.Array(rosidl_parser.definition.BasicType('int8'), 8),  # noqa: E501
        rosidl_parser.definition.Array(rosidl_parser.definition.BasicType('uint16'), 30),  # noqa: E501
    )
    ...
    def __init__(self, **kwargs):
        ...
        if 'bq_ntc' not in kwargs:
            self.bq_ntc = numpy.zeros(8, dtype=numpy.int8)
        else:
            self.bq_ntc = numpy.array(kwargs.get('bq_ntc'), dtype=numpy.int8)
            assert self.bq_ntc.shape == (8, )
        if 'mcu_ntc' not in kwargs:
            self.mcu_ntc = numpy.zeros(8, dtype=numpy.int8)
        else:
            self.mcu_ntc = numpy.array(kwargs.get('mcu_ntc'), dtype=numpy.int8)
            assert self.mcu_ntc.shape == (8, )
        if 'cell_voltage' not in kwargs:
            self.cell_voltage = numpy.zeros(30, dtype=numpy.uint16)
        else:
            self.cell_voltage = numpy.array(kwargs.get('cell_voltage'), dtype=numpy.uint16)
            assert self.cell_voltage.shape == (30, )
    ...
    def bq_ntc(self, value):
        if isinstance(value, numpy.ndarray):
            ...
            assert value.size == 8, \
                "The 'bq_ntc' numpy.ndarray() must have a size of 8"
            self._bq_ntc = value
            return
        if __debug__:
            ...
            assert \
                ...
                "The 'bq_ntc' field must be a set or sequence with length 8 and each value of type 'int' and each integer in [-128, 127]"
    ...
    def mcu_ntc(self, value):
        if isinstance(value, numpy.ndarray):
            ...
            assert value.size == 8, \
                "The 'mcu_ntc' numpy.ndarray() must have a size of 8"
            self._mcu_ntc = value
            return
        if __debug__:
            from collections.abc import Sequence
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, int) for v in value) and
                 all(val >= -128 and val < 128 for val in value)), \
                "The 'mcu_ntc' field must be a set or sequence with length 8 and each value of type 'int' and each integer in [-128, 127]"
    ...
    def cell_voltage(self, value):
        if isinstance(value, numpy.ndarray):
            assert value.dtype == numpy.uint16, \
                "The 'cell_voltage' numpy.ndarray() must have the dtype of 'numpy.uint16'"
            assert value.size == 30, \
                "The 'cell_voltage' numpy.ndarray() must have a size of 30"
        ...
        if __debug__:
            ...
            assert \
                ...
                "The 'cell_voltage' field must be a set or sequence with length 30 and each value of type 'int' and each unsigned integer in [0, 65535]"
        self._cell_voltage = numpy.array(value, dtype=numpy.uint16)
