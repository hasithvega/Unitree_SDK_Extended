    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'unitree_interfaces.msg.Obstacles')
            logger.debug(
...
class Obstacles(metaclass=Metaclass_Obstacles):
    ...
    def __init__(self, **kwargs):
        ...
        if 'obstacle' not in kwargs:
            self.obstacle = numpy.zeros(4, dtype=numpy.float32)
        else:
            self.obstacle = numpy.array(kwargs.get('obstacle'), dtype=numpy.float32)
            assert self.obstacle.shape == (4, )
    ...
    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if all(self.obstacle != other.obstacle):
            return False
        return True
    ...
    def obstacle(self, value):
        if isinstance(value, numpy.ndarray):
            assert value.dtype == numpy.float32, \
                "The 'obstacle' numpy.ndarray() must have the dtype of 'numpy.float32'"
            assert value.size == 4, \
                "The 'obstacle' numpy.ndarray() must have a size of 4"
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 True), \
                "The 'obstacle' field must be a set or sequence with length 4 and each value of type 'float'"
        self._obstacle = numpy.array(value, dtype=numpy.float32)
